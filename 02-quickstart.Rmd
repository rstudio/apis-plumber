...
This file defines two Plumber "endpoints." One is hosted at the path `/echo` and simply echoes the message passed in; the other is hosted at the path `/graph` and returns an image showing a simple R plot.
...
You should see a message about your API running on your computer on port `8000`. The API will continue running in your R session until you press the `Esc` key. If you're running this code locally on your personal machine, you should be able to open http://localhost:8000/echo or http://localhost:8000/graph in a web browser to test your new API endpoints. 
...
# Reach in to the private func and generate the image.
# Otherwise the processors route this into a file so we can't see it.
do.call(e[['.__enclos_env__']]$private$func, list(n=25, color="red"))
```

As you might have guessed, the `n=25` portion of the URL sets the `n` parameter to `25` and `color=red` specifies the color for the graph. More details on how Plumber processes inputs are available in the [Routing & Input Chapter](#routing-and-input).
...
You can even provide your own custom serializers and define how to translate the R object produced by your endpoint into the bits that will be used to respond to the incoming HTTP request.
