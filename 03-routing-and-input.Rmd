...
`args` | `"a=123"` | A list of the `QUERY_STRING` parameters and their values.
...
One common way of developing API's is to submit a form with its inputs using a POST request. For example, to send the following information to the server, we may have the following HTML inputs:
...
and  a `/submit` end-point as below:

```{r, echo=FALSE, results='asis'}
include_r("files/apis/03-06-args.R")
```

The `name` in the inputs correspond to the `names` of the `req$args` list passed to the Plumber endpoint.

To run/test the example above, just run the file in `files/apis/03-06-args.R` of the repo using `plumber` as follows:

```{r, eval=FALSE}
p = plumber::plumb("files/apis/03-06-args.R")
p$run(port = 8000)

# then visit
http://localhost:8000 # to see the form
# http://localhost:8000/submit is the endpoint
```
