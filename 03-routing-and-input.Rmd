...
If cookies are attached to the incoming request, they'll be made available via `req$cookies`. This will contain a list of all the raw cookies that were included with the request. See the [Setting Cookies section](#setting-cookies) for details on how to set cookies from Plumber.
...
HTTP headers attached to the incoming request are attached to the request object. They are prefixed with `HTTP_` and the name of the header is capitalized. 
