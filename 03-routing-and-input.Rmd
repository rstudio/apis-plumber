...
---- | ---- | ------- | --------------
`cookies` | `list` | `list(mycookie="abc")` | A list of the cookies as described in [Cookies](#read-cookies)
`httpuv.version` | `character` | `1.3.3` | The version of the underlying [`httpuv` package](https://github.com/rstudio/httpuv)
`PATH_INFO` | `character` | `/` | The path of the incoming HTTP request
`postBody` | `character` | `a=1&b=2` | The contents of the body of the request. Despite the name, it is available for any HTTP method.
`QUERY_STRING` | `character` | `?a=123&b=abc` | The query-string portion of the HTTP request
`REMOTE_ADDR` | `character` | `1.2.3.4` | The IP address of the client making the request
`REMOTE_PORT` | integer stored as a `character` | `62108` | The client port from which the request originated
`REQUEST_METHOD` | `character` | `GET` | The method used for this HTTP request
`rook.errors` | Reference class object | N/A | See https://github.com/jeffreyhorner/Rook/blob/a5e45f751/README.md#the-input-stream
`rook.input` | Reference class object | N/A | See https://github.com/jeffreyhorner/Rook/blob/a5e45f751/README.md#the-error-stream
`rook.url_scheme` | `character` | `http` | The "scheme" (typically `http` or `https`).
`rook.version` | `character` | `1.1-0` | The version of the rook specification which this environment satisfies
`SCRIPT_NAME` | `character` | "" | Unused
`SERVER_NAME` | `character` | `127.0.0.1` | The host portion of the incoming request. You may favor `HTTP_HOST`, if available.
`SERVER_PORT` | integer stored as a `character` | `8000` | The target port for the request
`HTTP_*` | `character` | `HTTP_USER_AGENT` | Entries for all of the HTTP headers sent with this request.
