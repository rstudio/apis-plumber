...
`args` | `"a=123"` | A named vector of the `QUERY_STRING`
...
One common way of developing API's is to submit a form with its inputs using a POST request. For example to handle a login form we would need: a form to send to the server such as:
...
and  a `/login` end-point as below:

```{r, echo=FALSE, results='asis'}
include_r("files/apis/03-06-args.R")
```

It is easy to see that the `name` in the inputs would be used to generate the named vector `args` in the `req` object passed to `plumber` API functions/end-points.
