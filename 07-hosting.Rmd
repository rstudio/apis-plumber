...
```bash
docker run --rm -p 8000:8000 trestletech/plumber \
  /usr/local/lib/R/site-library/plumber/examples/04-mean-sum/meansum.R
```
 ...
 - `/usr/local/lib/R/site-library/plumber/examples/03-mean-sum/meansum.R` is the path **inside of the Docker container** to the Plumber file you want to host. You'll note that you do not need plumber installed on your host machine for this to work, nor does the path `/usr/local/...` need to exist on your host machine. This references the path inside of the docker container where the R file you want to `plumb()` can be found. This `meansum` path is the default path that the image uses if you don't specify one yourself.
