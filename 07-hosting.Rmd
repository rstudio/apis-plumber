...
(These steps are tested on Ubuntu Server 18.04 LTS)

If you use a Linux server, both physical or virtual (e.g. Digital Ocean), you can use `systemd` to run Plumber as a service that can be accessed from your local network or even outside your network depending on your firewall rules.

This option is a bit similar to using Docker. One of the main advantages of this procedure over using Docker is that `systemd` won't bypass firewall rules (Docker does!) and it can be less costly in terms of resources.

Compared to `plumber::do_provision()` this option won't create a new droplet if you use Digital Ocean, and allows you to run your API on the same virtual machine where you have your databases and other materials. For example, you can write SQL queries from Plumber that point towards `localhost:5432` instead of going to a different IP from you API's server.

To implement this option you need three steps to be run from the command line:

1. Verify that you have plumber available for all users:
...
2. Run `sudo /etc/systemd/system/plumber-api.service`, paste and adapt these contents:
...
```
sudo systemctl activate plumber-api
