...
If you use a Linux server you can use `systemd` to run Plumber as a service that can be accessed from your local network or even outside your network depending on your firewall rules.

This option is similar to using Docker, as described above. One of the main advantages of this procedure over using Docker is that `systemd` won't bypass firewall rules (Docker does!) and avoids the overhead of running a container.
