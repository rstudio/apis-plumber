...
The R Programming Language [@R-base] has become one of the most dominant programming languages for data analysis and visualization in recent years. Simultaneously, allowing services to interact over the Internet has had a compounding effect on the usefulness of many services. The `plumber` R package [@plumber] allows users to expose existing R code as a service available to others on the Web. Plumber is best illustrated with an example:
...
Plumber makes use of these comment "annotations" above your functions to define the web service. When you feed the above file into Plumber, you'll get a runnable web service that other services or machines can interact with.
...
HTTP APIs have become the predominant language by which software communicates. By creating an HTTP API, you'll empower your R code to be leveraged by other services -- whether they're housed inside your organization or hosted on the other side of the world. Here are just a few ideas of the doors that are opened to you when you wrap your R code in a Plumber API:
 ...
 - You can have [a service](https://www.mailgun.com/) receive emails on your behalf and then notify your Plumber service when they arrive.
