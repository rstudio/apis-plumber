...
`@serializer unboxedJSON` | `application/json` | `jsonlite::toJSON(auto_unbox=TRUE)`
...
Sometimes it is good to have an end-point where a particular file lives. This could be a [mounted](#mount-static) directory where serveral files would live and querying any of these would "save" the file at the front end. However, this could be an R object that is generated on the fly and returned at a particular URL. For example, `http://localhost:8000/sample.pdf`. This could be achieved by: 
...
```
Then you can download the file from the `http://localhost:8000/sample.pdf` end-point and save it as `samaple.pdf`. The trick is to name your end-point (`/sample.pdf`) with the file name you would like your users to save their file in.
