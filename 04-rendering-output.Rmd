...
Sometimes it is good to have an endpoint where a particular file lives. This could be a [mounted](#mount-static) directory where serveral files would live and querying any of these would "save" the file at the front end. However, this could be an R object that is generated on the fly and returned at a particular URL. For example, `http://localhost:8000/docs/users.txt`. This could be achieved by: 

```{r, eval=FALSE}
#' Example to show how to specify a file name
#' Note modern browsers display text/plain instead of "downloading" it.
#' Saving the file will use the endpoint 'users.txt' file name.
#' 
#' @serializer contentType list(type="text/plain")
#' @get /docs/users.txt
download_users <- function () {
    content <- "plain text contents"
  # You can pass the serialization from above @serializer tag
  # by setting the header type yourself for instance:
  # add 'res' object to the function parameter above and
  # res$headers$`Content-type` <- "text/plain"
  # res$body <- content
  # then return the res object instead of 'content' object.
}
```
